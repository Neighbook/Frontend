...
export const addUserStory = async (description?: string): Promise<Post> => {
  const apiRes = await socialApi.post("story", {
    description,
  });
  if (apiRes.status === 200) {
    return apiRes.data as UserStory;
  }
...
export const addPostImage = async (
  ...
  const apiRes = await socialApi.post(
    "image/" + postId,
    { file },
    {
      headers: {
...
export const getUserPosts = async (
  ...
  const apiRes = await socialApi.get("posts", {
    signal: signal,
    params: { userId },
  });
  if (apiRes.status === 200) {
...
export const getPost = async (
  ...
  const apiRes = await socialApi.get("post", {
    signal: signal,
    params: { id },
  });
  if (apiRes.status === 200) {
...
export const postComment = async (
  ...
  const apiRes = await socialApi.post("comment", {
    idPost,
    contenu,
    idCommentaire,
  });
