...
export const filterOffres = async (data: Object, signal: GenericAbortSignal): Promise<Array<Offre> | null> => {
    const apiRes = await marketplaceApi.post("offres", data,{signal: signal});
    if(apiRes.status === 200) {
        return apiRes.data as Array<Offre>;
    }
    return null;
